"""
Looped Transformer for Parameter Golf.

Key idea: Instead of N unique transformer blocks, use K unique blocks looped L times
(K*L effective layers). This allows wider layers with fewer stored parameters.
The model learns per-iteration modulation so each "pass" through a block behaves differently.

Architecture: 3 unique blocks × 4 loops = 12 effective layers at dim 640.
"""

from __future__ import annotations  # Required for Python 3.9 compat with modern type hints
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,iter_gate,iter_gates",
    ).split(",")
    if pattern
)
...
class Block(nn.Module):
    def __init__(
        ...
        # Note: attn_scale and mlp_scale are shared across loop iterations.
        # Per-iteration modulation is handled by iter_gates in LoopedGPT.
        self.attn_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))
        self.mlp_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))

    def forward(self, x: Tensor, x0: Tensor, resid_mix: Tensor,
                iter_attn_gate: Tensor, iter_mlp_gate: Tensor,
        ...
        # Per-iteration gate modulates the shared attn_scale
        x = x + (self.attn_scale.to(dtype=x.dtype) * iter_attn_gate.to(dtype=x.dtype))[None, None, :] * attn_out
        x = x + (self.mlp_scale.to(dtype=x.dtype) * iter_mlp_gate.to(dtype=x.dtype))[None, None, :] * self.mlp(self.mlp_norm(x))
        return x


class LoopedGPT(nn.Module):
    """
    Looped Transformer: K unique blocks executed L times each.
    Total effective depth = K * L.

    Key novelty: Per-iteration gates allow each pass through a shared block
    to behave differently. Skip connections bridge encoder/decoder halves
    based on effective layer index.
    """
    def __init__(
        self,
        vocab_size: int,
        ...
        total_effective_layers = num_unique_blocks * num_loops

        self.tok_emb = nn.Embedding(vocab_size, model_dim)

        # Encoder-decoder split based on effective layer count
        self.num_encoder_layers = total_effective_layers // 2
        self.num_decoder_layers = total_effective_layers - self.num_encoder_layers
        self.num_skip_weights = min(self.num_encoder_layers, self.num_decoder_layers)
        self.skip_weights = nn.Parameter(torch.ones(self.num_skip_weights, model_dim, dtype=torch.float32))

        # The K unique blocks (shared across loops)
        ...
        # Per-iteration modulation: each effective layer gets its own gate pair.
        # These are cheap (2 * dim per effective layer) but allow each iteration
        # to scale attention and MLP outputs independently.
        self.iter_attn_gates = nn.Parameter(torch.ones(total_effective_layers, model_dim, dtype=torch.float32))
        self.iter_mlp_gates = nn.Parameter(torch.ones(total_effective_layers, model_dim, dtype=torch.float32))

        # Per-iteration residual mixing (separate from block's shared params)
        self.resid_mixes = nn.Parameter(
            torch.stack([
                torch.stack([torch.ones(model_dim), torch.zeros(model_dim)])
                for _ in range(total_effective_layers)
            ]).float()
        )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, lora=None) -> Tensor:
        ...
        for loop_idx in range(self.num_loops):
            for block_idx in range(self.num_unique_blocks):
                ...
                resid_mix = self.resid_mixes[effective_layer]
                iter_attn_gate = self.iter_attn_gates[effective_layer]
                iter_mlp_gate = self.iter_mlp_gates[effective_layer]

                qd = lora.q_loras[effective_layer] if lora else None
                vd = lora.v_loras[effective_layer] if lora else None

                # Encoder half: store skip connections
                if effective_layer < self.num_encoder_layers:
                    x = block(x, x0, resid_mix, iter_attn_gate, iter_mlp_gate, qd, vd)
                    skips.append(x)
                else:
                    # Decoder half: consume skip connections
...
class BatchedTTTLoRA(nn.Module):
    def __init__(self, bsz: int, model: LoopedGPT, rank: int):
        super().__init__()
        dim = model.tok_emb.embedding_dim
        vocab = model.tok_emb.num_embeddings
        total_effective_layers = model.num_unique_blocks * model.num_loops
        self.lm_head_lora = BatchedLinearLoRA(bsz, dim, vocab, rank)
        self.q_loras = nn.ModuleList()
        self.v_loras = nn.ModuleList()
        for i in range(total_effective_layers):
            block = model.blocks[i % model.num_unique_blocks]
...
def main() -> None:
    ...
    total_effective_layers = args.num_unique_blocks * args.num_loops
    ...
    log0(f"architecture:looped num_unique_blocks:{args.num_unique_blocks} num_loops:{args.num_loops} "
         f"effective_layers:{total_effective_layers} model_dim:{args.model_dim}")
    ...
    # Add looped-specific scalar params
    if base_model.skip_weights.numel() > 0:
        scalar_params.append(base_model.skip_weights)
    scalar_params.append(base_model.iter_attn_gates)
    scalar_params.append(base_model.iter_mlp_gates)
    ...
    log0(f"model_params:{n_params} (unique_block_params:{n_unique_block_params})")
