...
# FlashAttention-3 isn't available on sm_89 (RTX 4090). Fall back to torch SDPA.
class Hyperparameters:
    ...
    ttt_max_val_tokens = int(os.environ.get("TTT_MAX_VAL_TOKENS", 0))
    # Skip diagnostic eval, quantization roundtrip, and sliding-window eval.
    # When set, TTT runs directly on base_model with its current weights.
    skip_post_train_eval = bool(int(os.environ.get("SKIP_POST_TRAIN_EVAL", "0")))
    ttt_sparse_slots = int(os.environ.get("TTT_SPARSE_SLOTS", 256))
    ttt_topk = int(os.environ.get("TTT_TOPK", 8))
    recurrence_depth = int(os.environ.get("RECURRENCE_DEPTH", 3))
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        # SDPA expects (B, H, T, D); enable_gqa=True handles num_kv_heads < num_heads.
...
class SparseMemoryLayer(nn.Module):
    """CSM-TTT memory layer. `values` is the only thing TTT touches at eval time."""
    def __init__(self, dim: int, slots: int = 256, topk: int = 8):
        super().__init__()
        self.dim = dim
        self.slots = slots
        self.topk = min(topk, slots)
        self.q_proj = nn.Linear(dim, dim, bias=False)
        self.keys = nn.Parameter(torch.randn(slots, dim) / (dim ** 0.5))
        self.values = nn.Parameter(torch.randn(slots, dim) / (dim ** 0.5))
        self.o_proj = nn.Linear(dim, dim, bias=False)

    def forward(self, x: Tensor) -> Tensor:
        B, T, C = x.shape
        q = self.q_proj(x)
        sim = torch.einsum("btc,sc->bts", q, self.keys.to(q.dtype))
        topk_sim, topk_idx = torch.topk(sim, self.topk, dim=-1)
        weights = F.softmax(topk_sim, dim=-1)
        flat_idx = topk_idx.reshape(-1)
        gathered = self.values.to(q.dtype)[flat_idx].view(B, T, self.topk, C)
...
class Block(nn.Module):
    def __init__(
        ...
        self.mlp = SparseMemoryLayer(dim, slots=sparse_slots, topk=sparse_topk)
        ...
        if dtg:
            ...
            self.dtg_gate = None
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
...
class GPT(nn.Module):
    def __init__(
        ...
        # CSM-TTT: linear stack of `num_layers` physical blocks, looped `recurrence_depth` times.
        self.num_physical_blocks = num_layers
    ...
    def _init_weights(self) -> None:
        ...
        # Zero-init SparseMemoryLayer output proj so each block starts as identity.
        # The flag is honored by the generic nn.Linear init loop below.
        for blk in self.blocks:
            blk.mlp.o_proj._zero_init = True
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        v0 = None
        ve_cache: dict = {}
        for _r in range(self.recurrence_depth):
            for i in range(n):
...
def eval_val_sliding_ttt(
    ...
    # CSM-TTT: only update the SparseMemoryLayer `values` matrix in each block.
    ttt_params = []
    for name, param in base_model.named_parameters():
        if "mlp.values" in name:
            param.requires_grad_(True)
            ttt_params.append(param)
        else:
            param.requires_grad_(False)
    if not ttt_params:
        raise RuntimeError("CSM-TTT: no parameters matched 'mlp.values' \u2014 model wiring is wrong")
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        sparse_slots=args.ttt_sparse_slots,
        sparse_topk=args.ttt_topk,
    ...
    # SparseMemoryLayer 2D params (q_proj, o_proj weights, keys, values) live inside blocks.
    # They aren't banked, so they can't ride Muon. Route them to AdamW alongside scalars.
    sparse_mlp_params = [
        p for name, p in block_named_params
        if "mlp." in name and p.ndim >= 2
    ]
    scalar_params.extend(sparse_mlp_params)
    ...
    if args.skip_post_train_eval:
        log0("skip_post_train_eval=1: jumping past EMA/quant/sliding-eval and going straight to CSM-TTT")
        eval_model = base_model
        if args.ttt_enabled:
            torch.cuda.synchronize()
            t_ttt = time.perf_counter()
            ttt_val_tokens = val_tokens
            if args.ttt_max_val_tokens > 0 and val_tokens.numel() > args.ttt_max_val_tokens:
                ttt_val_tokens = val_tokens[:args.ttt_max_val_tokens]
                log0(f"ttt_subset: capping val_tokens to {ttt_val_tokens.numel()} (full={val_tokens.numel()})")
            ...
            torch.cuda.synchronize()
            log0(f"csm_ttt val_loss:{ttt_loss:.4f} val_bpb:{ttt_bpb:.4f} "
                 f"eval_time:{1000.0 * (time.perf_counter() - t_ttt):.0f}ms")
            log0(f"csm_ttt_exact val_loss:{ttt_loss:.8f} val_bpb:{ttt_bpb:.8f}")
        if distributed:
    ...
    eval_model = GPT(
        ...
        sparse_slots=args.ttt_sparse_slots, sparse_topk=args.ttt_topk,
        recurrence_depth=args.recurrence_depth,
    ).to(device).bfloat16()
    ...
    # CSM-TTT eval. ttt_chunk_tokens=64 over the full 62M val tokens is infeasible
    # on a single 4090, so allow capping val_tokens via TTT_MAX_VAL_TOKENS.
    if args.ttt_enabled:
        torch.cuda.synchronize()
        t_ttt = time.perf_counter()
        ttt_val_tokens = val_tokens
